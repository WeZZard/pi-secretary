...
try:
    ...
    checkpoint("07c-menu-subagents-section")
    start = len(raw)
    send("\r")
    expect("Edits the user-global configuration only.", start)
    checkpoint("07d-menu-manager")
    start = len(raw)
    send("a")
    expect("Name the fallback list.", start)
    send("acceptance-fallback")
    expect("acceptance-fallback", start)
    checkpoint("07e-menu-name-prompt")
    start = len(raw)
    send("\r")
    expect("Added list acceptance-fallback", start)
    checkpoint("07f-menu-list-added")
    start = len(raw)
    send("r")
    expect("Renaming preserves the list's models.", start)
    expect("> acceptance-fallback", start)
    checkpoint("07g-menu-rename-prompt")
    start = len(raw)
    send("-renamed")
    send("\r")
    expect("Renamed acceptance-fallback to acceptance-fallback-renamed.", start)
    checkpoint("07h-menu-list-renamed")
    start = len(raw)
    send("\r")
    expect("Models are tried from first to last.", start)
    checkpoint("07i-menu-list-detail")
    start = len(raw)
    send("\r")
    expect("Add Model › acceptance-fallback-renamed", start)
    ...
    checkpoint("07j-menu-model-picker")
    start = len(raw)
    send("\r")
    expect("Added secretary-tui-test/fixture to acceptance-fallback-renamed", start)
    checkpoint("07k-menu-model-added")
    start = len(raw)
    send("\x1b")
