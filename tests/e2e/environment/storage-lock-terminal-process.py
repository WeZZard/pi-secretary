#!/usr/bin/env python3
"""Drive interactive Pi over an owned PTY for the storage-lock reproducer.

Phases: wait for interactive startup, submit the goal-creation prompt, wait for
the turn to complete and the goal widget to mount, signal the test through a
marker file, idle for a bounded observation window (during which the test
holds an external write lock on the goals database), then quit cleanly — or
report that Pi died on its own.
"""
import argparse
import codecs
import errno
...
parser.add_argument("--widget-marker", required=True,
                    help="Written once the secretary goal widget is mounted and the turn has completed")
...
try:
    ...
    wait_for(lambda: any(e["event"] == "agent_end" and e["sessionId"] == parent_id for e in observations()),
             "goal creation turn completion", args.timeout)
    wait_for(lambda: any(e.get("event") == "widget" and e.get("id") == "secretary:goal" and e.get("mounted") is True
                         for e in probe_events()),
             "secretary goal widget mount", 30)
    checkpoint("widget-mounted")
    # The goal widget now re-renders every second. Signal the test to take the
    # external write lock, then keep pumping output for the observation window.
    ...
    with (out / "walkthrough.cast").open("w") as cast:
        cast.write(json.dumps({"version": 2, "width": columns, "height": rows,
                               "title": "Storage-lock widget render reproduction", "env": {"TERM": "xterm-256color"}}) + "\n")
        for event in events:
