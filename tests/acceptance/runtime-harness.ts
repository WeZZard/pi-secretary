...
export async function publicHarness(t: TestContext, options: { mode?: string; collision?: string; automatic?: boolean } = {}) {
  ...
  const emit = async (name: string, event: any = {}) => { let result: any; for (const fn of hooks.get(name) ?? []) result = await fn(event, ctx) ?? result; if (name === "session_shutdown") stopped = true; return result; };
  t.after(async () => {
