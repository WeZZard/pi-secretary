...
- Before the change, four deterministic reproducers failed: drilling into a delegating agent produced no transcript, the overlay never rendered a nested level, the terminal interface could not stop a nested run, and a nested outcome was never surfaced to the main session. The same file set now reports 6 tests, 6 passing, and 0 failing, including the guard that output retrieval already worked and a new two-level drill case.
