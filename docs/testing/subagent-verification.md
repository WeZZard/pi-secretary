...
**Reviewed implementation:** `352ccdd7cfd8d42bdb305a495927ca0c757118f6` with the nested-repair and outcome-acknowledgement changes summarized below. Earlier entries each name the revision they reviewed.

**Execution date:** 2026-09-23 for the two entries below. Earlier terminal and packaging observations are identified separately.
...
## 2026-09-23: Outcome acknowledgement and the undelivered-outcome projection

- The tested implementation is the working tree based on `dc36669`, implementing [architecture Section 3](../arch/subagents.md#3-invariants) Invariants 14 to 16, [Section 4.4](../arch/subagents.md#44-taskoutput), [Section 6.1](../arch/subagents.md#61-entities), and [Section 10](../arch/subagents.md#10-completion-delivery-and-model-context). The base commit alone does not contain this change.
- The defect was that the model-facing notice was keyed on the delivery transport state of the completion record rather than on whether the parent held the outcome. A settled run whose completion notification was never delivered stayed listed on every turn, and the notice instructed the parent to read it with `TaskOutput`, which could not change that state. One recorded session issued 268 `TaskOutput` calls, of which 206 were repeats against a single terminal run.
- The reproducer is `tests/agents/completion-ledger-contract.test.ts`. It drives the real extension through the host fixture, registers the real tools, and asserts the injected notice text and the stored acknowledgement. It needs no model and no network, and it runs in about one second.
- Before the change, the suite reported 3 passing cases and 4 failing cases, and its 25-case matrix reported 20 violations. The violations split into 15 cases where a terminal read neither retired a `pending`, `submitted`, or `uncertain` entry nor recorded anything, across all five terminal statuses, and 5 cases where a settled run with no completion record was omitted from the notice entirely.
- After the change, the suite reports 7 passing cases and 0 violations, and the three controls still pass. Those controls are that observing a delivered completion retires the run, that a read of a nonterminal run records no acknowledgement, and that an unsettled run is never an uninformed outcome.
- The settlement transaction was also completed in the interrupted-recovery path. [Section 10.1](../arch/subagents.md#101-delivery-protocol) already required that a run's terminal transition and its pending completion record are committed together, and that path previously violated the rule, which is what made a settled run invisible to the notice.
- `npm run verify` passed TypeScript checking, all 796 tests with no failures or cancellations, 31 Mermaid blocks, and acceptance syntax and traceability validation for 12 feature files and 112 scenario identities. The acceptance result is syntax and traceability validation rather than behavioral execution.
- Reproduce with `node --experimental-strip-types --test --test-concurrency=1 tests/agents/completion-ledger-contract.test.ts`, then `npm run test:subagents`, then `npm run verify`.
- Limits: the installed running extension was not reloaded, so any notice text quoted from a live session remains the pre-change wording. No live-provider run was performed, so the change is verified against deterministic fixtures only. The effect of the reworded notice on polling volume is not measured, because the recorded call volume came from a transcript study rather than from this suite. This section supersedes the `TaskOutput` coverage claim in the 2026-09-20 inline-rendering section, which described the behaviour before acknowledgement existed.

## 2026-09-23: Nested drill levels, nested stop, and promoted nested completion

- The tested implementation is the working tree based on `352ccdd7cfd8d42bdb305a495927ca0c757118f6`, implementing [the plan](../../.plans/2026-09-23-nested-drill-levels-nested-stop-and-promoted-completion.md) and the clauses added on the same date to [SA-12](../user-stories/subagents.md), [UX sections 3.1, 3.3, and 6](../ux/subagents.md), and [architecture sections 7.4, 10.1, 10.2, 11.2, 12.1.6, 12.5, 12.6.3, and 14](../arch/subagents.md). The base commit alone does not contain these changes.
- Before the change, three deterministic reproducers failed against the existing requirement that the overlay present each agent's children as a drill-down level: drilling into a delegating agent produced no transcript, the overlay never rendered a nested level, and the terminal interface could not stop the nested run it displayed. A fourth case, a settled nested outcome whose owning session has ended, is not a defect against the original specification; it is a proposed addition aligned with Claude Code, and it is recorded as `ACC-SA-01-09` under the `@proposed` tag. The same file set now reports 6 tests, 6 passing, and 0 failing, including the guard that output retrieval already worked and a new two-level drill case.
- The row-retention defect was confined to the interface. `installation.ts` already supplied `AgentService.tree()` to the user-interface port, and the reducer then discarded nested records with a session-identity filter. The reducer now retains the session's own children plus every descendant reachable through parent agent identity, so a drill level is derived from the delegation tree rather than from session ownership.
- Action eligibility compared the capturing session with the agent's owning session, which rejected every nested row. Eligibility is now evaluated against the agent record, and the service continues to route a nested stop to the owning session. The interface therefore stops exactly the selected nested agent, cancels the agents it launched, and leaves its parent and its siblings running.
- Completion delivery read only the current session's rows. The outcome projection and the acknowledgement written by a terminal read now cover the sessions this session may deliver for: its own, plus descendant sessions whose service has ended. A nested outcome is therefore promoted to the nearest live ancestor, with the main session as the final recipient. Promotion is expressed over the acknowledgement model recorded in the entry above rather than over a separate delivery state, leaves the record's run identity and parentage unchanged, and an informed outcome is not presented again. The promotion recipient is derived from owner liveness at read time rather than stored in a new column. This is an addition to the original delivery protocol, which delivered an outcome to the owning parent only; the requirement did not previously exist, and `ACC-SA-01-09` carries the `@proposed` tag rather than describing a repair.
- `npm run verify` passed TypeScript checking, all 805 tests with no failures or cancellations, Mermaid validation for 31 of 31 blocks, and acceptance syntax and traceability validation for 12 feature files and 115 scenario identities. The count is higher than either side of the merge because it covers the merged tree, including the acknowledgement work recorded in the entry above. The generated log is retained at the ignored path `test-results/nested-plan-verify/verify-merged.log`; it is a local artifact and not a versioned baseline.
- Reproduce the repaired checks with `node --experimental-strip-types --test --test-concurrency=1 tests/agents/nested-ui-repro.test.ts tests/agents/nested-output-repro.test.ts`, then run `npm run verify`. The reproduction does not require a paid provider or the user's stored data.
- New executable acceptance scenarios are ACC-SA-02-15 for a two-level drill and return, ACC-SA-04-08 for stopping one nested agent from its drill level, and ACC-SA-01-09 for delivering a nested outcome after its delegating session has ended. ACC-SA-02-13 was corrected so its nested records carry the owning session identity, which is the shape the service produces; the previous fixture reused the root session identity and could not detect the retention defect.
- Verification uses the deterministic fixture provider and the installed host. It does not establish live-provider behaviour, human visual approval, or multi-host terminal compatibility. Nesting is exercised to three levels; deeper trees, concurrent nested stop, and promotion across a still-live intermediate owner remain unverified.

## 2026-09-20: Goal-independent subagents and external composition

- The tested implementation is the working tree based on `66feb644cd32e98d555d6b1f4a72d1f61de964e3`, implementing the [decoupling plan](../../.plans/2026-09-20-goal-subagent-decoupling.md) and [composition contract](../arch/goal-agent-composition.md). The base commit alone does not contain this repair.
- Before production changes, the real parent/child SDK regression in `tests/integration/goal-agent-composition.test.ts` failed for both `Agent` and `SendMessage` with `This goal does not authorize new delegated work.` The initial child had successfully completed, and the next request was genuine RPC-source user input after `update_goal` blocked the goal. The same regression now observes both recovery executions completing while the goal stays blocked and no automatic goal continuation restarts.
- Real-SDK standalone tests install delegation without a goal service or goal schema. They cover foreground execution, usage recording without consumers, running guidance, saved-child resumption, cancellation, nested delegation, and root guidance to a nested child. Dependency tests inspect direct and transitive imports, public identifiers, and hard-coded capabilities for goal coupling.
- Real-SDK composition tests cover user requests with active, blocked, paused, complete, usage-limited, and budget-limited goals; delayed automatic responses; cancellation of automatic runs; survival of user-owned runs; guidance cancellation ownership; fresh input alongside deferred completion data; reporting-only budget wrap-up; and refusal of goal mutations from notifications, including a retry after a transient provider error.
- Migration tests cover immutable mappings, reopening, historical run and usage attribution, malformed/conflicting payloads, transaction rollback after injected failure, idempotent retry, retained conversation/output references, and unchanged usage deduplication markers. Accounting tests cover mapping-observer failure and root restoration replaying descendant owners without charging unrelated goals or double-counting events.
- The ordinary host-tool admission limit is explicitly exercised: a pause before Secretary's `tool_call` check rejects the call; a pause during a later third-party hook may allow an already-admitted call to finish. Both cases preserve queued user input without blanket parent abort. This does not establish a host-wide lock through actual tool execution. Secretary-owned delegation additionally rechecks execution and asynchronous admission with cancellation signals.
- Executable ACC-SA-08-10 and ACC-SA-08-11 bindings cover blocked-goal recovery and standalone operation. Older accounting scenarios now distinguish automatic goal authority from ordinary user-directed assignments.
- `npm run verify` passed TypeScript checking, all 771 tests with no failures or cancellations, Mermaid validation, and acceptance syntax/traceability validation for 12 feature files and 112 scenario identities. The acceptance lint result is separate from behavioral execution.
- Reproduce the incident with `node --experimental-strip-types --test tests/integration/goal-agent-composition.test.ts`. Reproduce the broader boundary checks with `node --experimental-strip-types --test --test-concurrency=1 tests/agents/dependencies.test.ts tests/agents/standalone.test.ts tests/integration/agent-*.test.ts tests/integration/composition-host-boundary.test.ts`, then run `npm run verify`.
- Each regression run retains evidence in a fresh ignored directory under `test-results/`; the real-SDK composition fixture prints its evidence directory and saves execution details and isolated session copies. To retain a complete development-gate log, create a fresh directory with `mktemp -d test-results/decoupling-verify-XXXXXX` and redirect `npm run verify` output there. These files are not versioned baselines.
- Verification uses deterministic providers and the installed Pi SDK, not a paid-provider matrix or human acceptance. The user's live database and installed running extension were not migrated or reloaded. Migration was executed against isolated fixtures; this report does not claim live deployment, a new GUI walkthrough, or power-loss testing of a real database.

## 2026-09-20: Inline Agent and SendMessage compact/full rendering

- The tested implementation is the working tree based on `5c18374`, implementing [UX Section 2.1](../ux/subagents.md#21-inline-tool-display) and [architecture Section 12.6.7](../arch/subagents.md#1267-inline-compactfull-rendering-contract). The working tree includes uncommitted documentation, test, and runtime changes; the base commit alone does not contain this implementation.
- `npm run verify` passed TypeScript checking, all 701 tests with no failures or cancellations, Mermaid validation, and acceptance syntax/traceability validation for 11 feature files and 110 scenario identities. The revised ACC-SA-UI-15 binding checks compact/full behavior rather than the removed display modes.
- Registered-host tests cover wireframes A–I, actual sent-message previews, first-result header composition, same-row progress and compact/full transitions, completion, replay, error visibility, and retention of previously observed identity after an unstructured error. Compact C/F selection is asserted with zero versus nonzero turn/tool counts. Legacy `TaskOutput` and `TaskStop` behavior remains covered.
- Component tests cover independent 199/200/201 wrapped-line boundaries, resize, Unicode and terminal-control input, unavailable artifacts, original multiline content, and bounded metadata. Artifact tests verify exact original text, private permissions, operation separation, idempotent retries, refusal to follow symlinks or reuse mismatched content, queued-versus-resumed acknowledgment, and non-fatal retention failure after accepted guidance.
- `python3 scripts/record-inline-tools.py` passed an isolated real-Pi PTY walkthrough with 11 replay-verified terminal-grid checkpoints. It exercised keyboard compact/full toggling during foreground progress and after completion, successful background launch, compact/full sent-message content, and narrow/wide resize. `scripts/check-inline-recording.ts` replays recorded output through xterm and asserts visible rows; it does not substitute generated mock screens for application execution.
- The initial full gate exposed a session-shutdown regression caused by checking message correlation before controller availability. Restoring the original check order fixed the four affected departure cases; the final full gate passed. An initial fixture run also needed to wait for the first child stream before queueing guidance so that its expected second turn was deterministic.
- Reproduce component checks using the commands in [the inline test guide](inline-tool-wireframes.md), and record a fresh walkthrough with `python3 scripts/record-inline-tools.py`. The recorder prints its new ignored output directory and writes `result.json`, `run-metadata.json`, the terminal cast, and replayed grid assertions. Test logs and recordings remain under ignored `test-results/`; they are not versioned baselines.
- These checks use deterministic providers and the installed Pi 0.85.1 host. They do not establish live-provider fallback behavior, a broader host/terminal compatibility matrix, pixel rendering in a desktop terminal, or human visual approval. No paid-provider E2E run or human visual approval is claimed.
...
| --- | --- | --- |
| Tool registration and parent lifecycle. | `installation.ts` and `service.ts` implement independent delegation. | `tests/agents/installation.test.ts`, `standalone.test.ts`, `dependencies.test.ts`, and `service.test.ts` exercise the boundaries. |
| External goal composition and accounting. | `extensions/secretary/composition/` coordinates public operations and events outside the agent module. | `tests/integration/goal-agent-composition.test.ts`, `agent-accounting.test.ts`, `agent-association-store.test.ts`, and `agent-request-ordering.test.ts` cover the revised contract. |
