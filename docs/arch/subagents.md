...
- Tool vocabulary is narrower than "every tool pi can construct". pi's built-in tool names are `read`, `bash`, `powershell`, `edit`, `write`, `grep`, `find`, and `ls`, while the default active set is `read`, `bash`, `edit`, `write`, overridable through the host's `defaultTools` setting. `grep`, `find`, and `ls` are therefore real pi tools that a default session does not activate, which is why a definition naming one loses it silently rather than failing the launch. Verified against pi 0.87.1 (`dist/core/tools/index.js` `allToolNames`; `dist/core/sdk.js` and `dist/core/agent-session.js` `defaultActiveToolNames`).
...
- `Explore` and `Plan` declare `disallowedTools: edit, write` and no `tools` allowlist. They retain `bash`, because that is how a default pi session searches. `bash` is consequently a second route to the filesystem, and the packaged prompts state the read-only prohibition.
...
- A `tools` allowlist remains supported for custom definitions that need a hard capability bound, but it is the wrong instrument for read-only intent. An allowlist fails closed and silently: one name the session does not have active removes capability, with no diagnostic and no effect on the launch's outcome record. A denylist fails open, and a name the host does not recognize is inert.
...
- The catalog and its execution configuration are captured fresh on every request. A configuration or definition edit persisted before a request is reflected in that request's launches; an edit made while a request is in flight applies from the next request. Launches never resolve against a cached previous request.
...
- The transcript viewport, scrolling, and follow behavior of Sections 12.1.3 and 12.4 render the structured events of Section 12.6.2. A fixed status header renders above the viewport: name, status label, and stats on the first line, current activity on the second, and a single divider between the header and the viewport. The header is not part of the scroll anchor or the follow state, and the follow state is not restated as a separate detail row because the viewport position already shows it. The viewport paints only the events that intersect the visible window, so paint cost is bounded by the viewport rather than by transcript length. Tool-detail expansion toggles the bounded argument and output blocks. In the mouse-enabled alternate-screen host, SGR wheel events over the transcript pane scroll the transcript and wheel events over the list move the selection; both reuse the existing state machines and add no states. Dialogs return a handled result for wheel events so the host cannot forward the raw mouse sequence into the text input. Main-screen mode retains terminal-owned wheel scrolling; keyboard scrolling remains available in both modes.
- The footer enumerates only the actions available for the selected record and reflects configured keybindings. It renders one hint row in a fixed published order and omits whole actions by ascending priority when the width cannot accommodate every action; close, stop, and page-scroll are omitted last. The collapsed tool preview names the resolved expansion key, never the stop shortcut.
...
The `/secretary` configuration menu edits the user-global `secretary.json`. Each menu mutation is an operation — add a list, rename a list, add, move, or remove a model — applied to the lists freshly re-read at the persistence boundary, never to the menu's older in-memory copy, so a menu action cannot revert a newer edit made by another session or by hand. An operation whose target no longer matches the fresh state, such as renaming a list that was removed elsewhere, fails with a not-saved status and leaves the file unchanged. The full resulting `agents` object is validated against the same rules as file loading, including unknown-key rejection, before writing; a failed validation or write leaves the previous configuration in effect. Project-level overrides are not edited through the menu in this release. Menu navigation and editing keys are fixed and are specified in the [interaction design's navigation table](../ux/subagents.md#4-navigation-and-accessibility).
